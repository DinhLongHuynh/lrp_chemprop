...
class LRP_rules:
    ...
    def positive_matmul(self, A, B):
        """
        Performs matrix multiplication with positive and negative components.
        """
    ...
    def negative_matmul(self, A, B):
        """
        Performs matrix multiplication with positive and negative components for negative relevance.
        """
        A_plus = torch.clamp(A, min=0)
        B_plus = torch.clamp(B, min=0)
        A_minus = torch.clamp(A, max=0)
    ...
    def lrp_dense_epsilon(self, activation_j, relevance_k, weights_matrix, bias_matrix=None, epsilon=0):
        """
        LRP for dense layers with epsilon stabilization.
        """

        if not torch.is_tensor(activation_j) or not torch.is_tensor(relevance_k):
            raise TypeError("Inputs must be PyTorch tensors")
        if activation_j.dim() != 2 or relevance_k.dim() != 2:
            raise ValueError("Inputs must be 2-dimensional tensors")
    ...
    def lrp_dense_ab(self, activation_j, relevance_k, weights_matrix, alpha=1.0):
        """
        LRP for dense layers with alpha-beta rule.
        """
        ...
        weights_plus = torch.clamp(weights_matrix, min=0.0)  # Positive weights
        weights_minus = torch.clamp(weights_matrix, max=0.0)  # Negative weights
        activation_plus = torch.clamp(activation_j, min=0.0)  # Positive activations
    ...
    def lrp_aggregation_epsilon(self, activation_j, relevance_k, batch_index, epsilon=0, agg_func='sum'):
        """
        Optimized vectorized implementation of LRP through an aggregation layer.
        """
